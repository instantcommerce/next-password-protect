        ...
        <script>
          ...
          function onSubmit(e) {
            ...
            fetch("${apiUrl || '/api/login'}", {
              method: 'post',
              credentials: 'include',
              body: JSON.stringify({ password:  document.querySelector(
                '#password-form .input'
              ).value }),
      ...
      <body>
        <main>
          <div class="container">
            ${
              !!image
                ? `${backUrl ? `<a href="${backUrl}">${image}</a>` : image}`
                : ''
            }
          ...
          <script>
            document.querySelector("#password-form form").addEventListener("submit", onSubmit);
          </script>
        </main>
      </body>
    </html>
